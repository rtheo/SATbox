...
[overlaps,  pows, deg] = satassign( atoms, nots, L, La, nc );
if method, 
...
if option   
    disp('Negation Code input ignored for 2nd arg = 1')
    dnf = zeros( dim );cnf = dnf;     
    for nots = 0:dim-1
        [c d] = sateval( nots, pows, deg, int, dim - int - 1, cp, cc, nc );
        cnf(nots+1, :) = c; dnf(nots+1, :) = d; 
    end
    graph( cnf, dnf, L, 1 );
else
    nots = bin2dec( num2str( floor( (nots + 1)/2 ) ) ); %floor( (nots + 1)/2 )*(2.^(0:L-1))';
    [c, d] = sateval( nots, pows, deg, int, dim - int - 1, cp, cc, nc ); graph( c, d, L, 0 ); 
end
end
