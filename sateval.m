...
if isempty( over )
    ...
    psi = filtrate( int, notint, over, Lo, dim );
    Filter = find( psi == 1 ); 
...
function psi = filtrate( int, notint, over, Lo, dim )
psi1 = zeros(1, dim); psi2 = psi1; periods = 2.^(over - 1); 
for i=1:Lo, psi1 = psi1.*eta( int, periods(i), periods(i), 1 ); psi2 = psi2.*eta( notint, periods(i), periods(i), 1 ); end
psi = psi1 + psi2;
end
